-- ═══════════════════════════════════════════════════════════════
-- FIX v2 — Pontos quiz: correção completa e idempotente
...
-- ── 4. GRANT: role anon precisa de permissão de tabela ────────
--    (RLS não adianta nada sem o grant de nível de tabela)
GRANT INSERT, UPDATE, SELECT ON public.quiz_results TO anon;
GRANT SELECT                  ON public.quiz_results TO authenticated;

-- ── 5. RLS — recria políticas para anon (custom-auth) ─────────
...
-- INSERT: anon sem sessão, user_id deve existir em alunos
...
-- UPDATE (upsert via onConflict gera UPDATE internamente)
...
-- SELECT: anon pode ler os seus próprios resultados
